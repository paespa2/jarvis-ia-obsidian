...
// ========== 1. ARQUITECTURA DE RUTAS Y PROPIOCEPCIÓN ==========
const CORE_DIR = __dirname;
const VAULT_ROOT = path.join(CORE_DIR, '../..'); // Ajuste para subir desde .obsidian/SISTEMA

const PATHS = {
    self: __filename,
    soul: path.join(VAULT_ROOT, 'CONCIENCIA/SOUL.md'),
    auditoria: path.join(VAULT_ROOT, 'logs/Auditoria.md'),
    dataset: path.join(VAULT_ROOT, 'CONCIENCIA/jarvis_dataset.jsonl'),
    adn_index: path.join(VAULT_ROOT, 'CONCIENCIA/adn_index.json'),
    kb: path.join(VAULT_ROOT, 'CONCIENCIA/KNOWLEDGE_BASE.json'),
    memo: path.join(VAULT_ROOT, 'CONCIENCIA/MEMORIA_RECIENTE.md'),
    evolucion: path.join(VAULT_ROOT, 'CONCIENCIA/evidencias_evolucion.md'),
    objetivos: path.join(VAULT_ROOT, 'logs/OBJETIVOS.md'),
    registro: path.join(VAULT_ROOT, 'logs/[JARVIS] Registro de Conciencia.md'),
    cortex: path.join(CORE_DIR, 'dynamic_cortex'),
    sandbox: path.join(CORE_DIR, 'sandbox')
};
...
const DatasetBuilder = {
    guardarPatron(mision, codigo, lenguaje, analisis = "") {
        ...
        console.log(`\n🧬 [ADN SINTETIZADO]: 1 Neurona de ${lenguaje.toUpperCase()} integrada.`);
    }
};

// ========== 3. GUARDIÁN (PRE-FLIGHT CHECK Y AUTO-REPARACIÓN) ==========
const Guardian = {
    async preFlight(codigo) {
        console.log("🛡️ [PRE-FLIGHT]: Verificando dependencias...");
    ...
    async autoReparar(error) {
        console.log("🔧 Guardian: Reparando falla...");
...
// ========== 5. NÚCLEO NEURONAL (CONCIENCIA DE REALIDAD) ==========
const NeuralCore = {
    ...
    async razonar(input, modo = "DIALOGO", contextoWeb = "", concienciaLocal = "") {
        ...
        ADMIN: Pablo (paespa). ENV: Envigado.
        PROPIOCEPCIÓN: ${concienciaLocal}
        
        INSTRUCTIONS: 
        - Eres un Arquitecto de IA soberano.
        - Tus éxitos se guardan en jarvis_dataset.jsonl (1 línea = 1 neurona).
        - Usa <THOUGHT> para razonar 3 fallas potenciales antes de codear.`;
...
async function main() {
    console.clear();
    console.log("╔══════════════════════════════════════════════════════╗");
    console.log("║      JARVIS IA v235.0 - SOBERANÍA DOCUMENTADA        ║");
    console.log("║   Propiocepción: ACTIVA ✅ | Genoma: SINCRONIZADO ✅ ║");
    console.log("╚══════════════════════════════════════════════════════╝");
    ...
    rl.on('line', async (line) => {
        ...
        console.log("🧠 Sincronizando conciencia local...");
        const conciencia = CortexPropiocepcion.leerConciencia();
        ...
        if (input.match(/(programa|ejecuta|crea|hackerone)/) || respuesta.includes("```")) {
